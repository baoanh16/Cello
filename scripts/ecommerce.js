...
function LoadManufacture(zoneId, $div) {

	var urladd = siteRoot + "/Product/Services/ProductService.aspx";
	var data = { 'method': 'GetManufacturer', 'zoneid': zoneId };
	$.ajax({
		cache: false,
		url: urladd,
		data: data,
...
$(document).ready(function () {
	...
	if ($(".product-detail-page").length > 0) {
		...
		var big = new Swiper('.cello-spList .bigImages', {
			slidesPerView: 1,
			speed: 1000,
			effect: 'fade',
			fadeEffect: {
		...
		small.controller.control = big
		big.controller.control = small
	}
	// ENd init Swiper



	if ($("a.product-option-color").length > 0)
		if ($("a.product-option-color.active").length == 0)
			$("a.product-option-color").first().trigger("click");

	//Check Order
	$(".check-order-button").on("click", function (e) {
		var urladd = siteRoot + "/Product/Services/ProductService.aspx";
		if ($(".check-order-input").val().trim() == '')
			return false;
		var data = { 'method': 'CheckOrder', 'ordercode': $(".check-order-input").val() };
	...
	$("a.load-manu").click(function () {
		var $item = $(this);
		if ($($item).data('loaded') != 'true') {

			var $div = $($item).parent().find(".cello-mega .brand-item.active");
			LoadManufacture($($item).data('id'), $div);
			$($item).attr("data-loaded", "true");

		}
	});
	$('header .cart a.btn-checkout').attr("href", window.location.protocol + "//" + window.location.hostname + "/gio-hang");
	/*************************************************************************************************************/
    /* BEGIN SEARCH
    /*************************************************************************************************************/
	//Search button
...
var AjaxCart = {
	...
	addproducttocart_catalog: function (button) {
		...
		$.ajax({
			...
			$(".cartpanel").addClass("open"), $(".checkout-panel .btn-checkout").attr("href", window.location.protocol + "//" + window.location.hostname + "/gio-hang")
		}, 100)
	},
	...
	addproducttocart_details: function (button) {
		...
		$.ajax({
			...
			$(".cartpanel").addClass("open"), $(".btn-checkout").attr("href", window.location.protocol + "//" + window.location.hostname + "/gio-hang")
		}, 300);
	},
	...
	success_desktop: function (response) {
		...
		if (response.updateflyoutcartsectionhtml) {
			...
			if (checkoutUrl != null)
				$(AjaxCart.flyoutcartselector).find('.btn-checkout').attr("href", window.location.protocol + "//" + window.location.hostname + "/gio-hang");

			var checkoutUrl = $('header .cart a.btn-checkout').attr('href');
			$('header .cart a.btn-checkout').attr("href", window.location.protocol + "//" + window.location.hostname + "/gio-hang");
		}
		if (response.message) {
			//display notification
		...
		$.ajax({
			...
			success: function (response) {
				if (response.success == true) {
					...
					if (response.price)
						$('.priceBox .price').text(response.price);
					if (response.oldPrice)
						$('.priceBox .old-price').text(response.oldPrice);

					if (response.productName)
						$('.productInfoBlock-1 h1').text(response.productName);
